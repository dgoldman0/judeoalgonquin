...
The project is currently in its **foundation / pre-corpus** stage. No legacy translation is canonical. The old fine-tuning-era tree remains recoverable from the archival Git tags documented in [the legacy audit](docs/legacy-audit.md).
...
- The first paid embedding call is a small smoke test. Further paid API work requires a report to and approval from the project owner.
...
Python 3.12 or newer is required. The core validator and local search use the standard library. A live embedding call additionally requires the OpenAI Python SDK.

```bash
python -m venv .venv
.venv/bin/python -m pip install "openai>=2.15,<3"
...
Paid embedding and semantic-search paths require both `--live` and an enabled, capped authorization in [`config/api-budget.json`](config/api-budget.json). The one foundation smoke call has already been consumed and cannot be repeated by the committed CLI state.
