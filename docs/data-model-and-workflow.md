...
The first live embedding operation was limited to one small smoke test over noncanonical fixtures. Its committed report acts as a persistent consumed marker. Further live embedding or semantic-search operations require `--live`, an explicitly enabled `config/api-budget.json`, per-run caps, a cumulative token cap, and an ignored local reservation/usage ledger. Reservations are written before network access so a crash or uncertain failure cannot silently invite an unaccounted retry.
